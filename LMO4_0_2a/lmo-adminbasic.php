...
<table class="lmoMiddle" cellspacing="0" cellpadding="0" border="0">
  ...
  if ($_SESSION['lmouserok']==2 || $_SESSION['lmouserokerweitert']==1) {?>
    ...
    if ($lmtype==0){?>
        ...
        <tr><td align="right"><a href='<?php="$addr-3"?>' onclick="return chklmolink();" title="<?php echo $text[339]?>"><?php echo $text[338]?></a></td></tr><?php
    }
  }?>
      </table>
    </td>
  ...
  if ($show==0) {
    ...
    if($_SESSION['lmouserok']==2 || $_SESSION['lmouserokerweitert']==1){ ?>
      ...
      for($y=0;$y<count($dummf);$y++){?>
                <option value="<?php echo $dummf[$y]?>"<?php if($datf==$dummf[$y]){echo " selected";}?>><?php=strftime($dummf[$y])?></option><?php
      }?>
              </select>
            </td>
          </tr>
