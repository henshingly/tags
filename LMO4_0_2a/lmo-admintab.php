...
/** Liga Manager Online 4
  ...
  *
  * $Id: lmo-admintab.php 389 2007-02-25 10:03:06Z jokerlmo $
  */
  ...
  for($x = 1; $x <= $anzteams; $x++) {
    ...
    if ($teamn[$i]!="" || $strafp[$i]!=0 || $strafm[$i]!=0 || $torkorrektur1[$i]!=0 || $torkorrektur2[$i]!=0) {
      ...
      if ($teamn[$i]!="") {
        ...
        <a href='#' onclick="alert('<?php=mysql_escape_string(htmlentities(strip_tags($lmo_tabellennotiz)))?>');window.focus();return false;"><img src='<?php echo URL_TO_IMGDIR."/lmo-st2.gif"?>' width='10' height='12' border='0' alt=''><span class='popup'><?php=nl2br($lmo_tabellennotiz)?></span></a><?php
      $lmo_tabellennotiz="";
    } else {
      echo "&nbsp;";
    }?>
    ...
    if ($tabpkt == 0) {?>
            <td width="2">&nbsp;</td>
            <td align="right"><strong><?php=applyFactor($punkte[$i],$pointsfaktor)?></strong></td><?php
      if ($minus == 2) {?>
            <td align="center" width="4"><strong>:</strong></td>
            <td><strong><?php=applyFactor($negativ[$i],$pointsfaktor)?></strong></td><?php
      }
    }?>
            <td>&nbsp;</td>
