...
if ($_SESSION['lmouserok']==2) {
  ...
  if ($save>=0) {
    if(isset($_POST["xadmin_name".$save]) && isset($_POST["xadmin_pass".$save]) && isset($_POST["xadmin_rang".$save])) {
      ...
      $lmo_admin_data[$save][3]=implode(',',$lmo_helfer_ligen_neu); //Ligen hinzufï¿½gen
    ...
    if (empty($admin_dupe) && !empty($_POST["xadmin_name"]) && !empty($_POST["xadmin_pass"])){
      $lmo_admin_data[]=array(trim($_POST["xadmin_name"]),trim($_POST["xadmin_pass"]),1,'','');  //Neue User sind zunï¿½chst immer Hilfsadmins ohne Ligen
      require(PATH_TO_LMO."/lmo-saveauth.php");
      require(PATH_TO_LMO."/lmo-loadauth.php");
...
//Sortierfunktion fï¿½r die Admins
