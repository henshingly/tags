...
if ($file!="") {
  ...
  //  if($lmtype==1 && $modus[$st-1]!=2){$hidr=1;} // bei KO-Turnier auï¿½er bei Hin- und Rï¿½ckspiel keinen Remistipp zulassen
  if ($hidr==1) {
    $breite--;
  }
  if ($tipp_tippmodus==0) {
...
/**ERGEBNISMODUS*/
  if($tipp_tippmodus==1){  
    if($tipp_showdurchschntipp==1){ ?>
            ...
            <?php echo "ï¿½-".$text['tipp'][30]; /* DurchschnittsTipp*/ ?>
            </th><?php
    } ?>
    ...
    if ($teama[$st-1][$i]>0 && $teamb[$st-1][$i]>0) {
      if ($lmtype==0) {
        ...
        require(PATH_TO_ADDONDIR."/tipp/lmo-tippeditko.php");
          ...
          <tr>
            <td colspan="<?php echo $datm*2+10-$hidr; ?>" align="right"><?php
  if($tipp_imvorraus>=0 && $st>($stx+$tipp_imvorraus)){
    echo $text['tipp'][177];
  } 
  if($savebutton==1){ ?>
              <input class="lmo-formular-button" type="submit" title="<?php echo $text[114] ?>" name="best" value="<?php echo $text['tipp'][8]; ?>"<?php  if($tipp_imvorraus>=0 && $st>($stx+$tipp_imvorraus)){echo " disabled";} ?>><?php
  }else{
    echo "&nbsp;";
  }?>
            </td>
